...
static int write_decimal_as_string(unsigned char *buf, void *value, int bytesize, int scale) {
    ...
    if (val < 0) { str[len++] = '-'; abs_val = -val; } else abs_val = val;
    do { tmp[i++] = (char)(abs_val % 10 + '0'); abs_val /= 10; } while (abs_val > 0);
...
void ExportToTrino_contract(INTEGER *Result, int *indicator_Result, char sqlstate[6], SQL_TEXT extname[129], SQL_TEXT specific_name[129], SQL_TEXT error_message[257]) {
    ...
    char mycontract[] = "ExportToTrino v4.1";
    ...
    oCols->column_types[6].datatype = VARCHAR_DT; oCols->column_types[6].bytesize = 258; oCols->column_types[6].size.length = 256;
...
void ExportToTrino(void) {
    ...
    FNC_TblOpGetAmpId(&stats.amp_id);
    parse_params(&params);
    FNC_TblOpGetStreamCount(&incount, &outcount);
    ...
    if (connect(sock_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        stats.error_code = errno; strcpy(stats.error_message, "Connect failed"); goto cleanup;
    }
    /* Handshake */
    unsigned char hdr[1024]; int ho = 0; int ql = strlen(params.query_id);
    ho += write_uint32(hdr + ho, ql); memcpy(hdr + ho, params.query_id, ql); ho += ql;
    char sj[4096]; strcpy(sj, "{\"columns\":[");
    ...
    /* Data Loop */
    while (FNC_TblOpRead(icolinfo[0].Handle) == TBLOP_SUCCESS) {
        in = icolinfo[0].Handle; stats.rows_processed++; rows_in_batch++;
        for (col = 0; col < icolinfo[0].colcount; col++) {
            batch_buffer[batch_offset++] = TBLOPISNULL(in->row->indicators, col) ? 1 : 0;
            if (TBLOPISNULL(in->row->indicators, col)) stats.null_count++;
                ...
                else if (dt == 3 || dt == 4) batch_offset += write_int32(batch_buffer + batch_offset, (dt==3?*(short*)val:*(__int8_t*)val));
...
cleanup:
    ...
    r_amp = stats.amp_id; r_rows = stats.rows_processed; r_bytes = stats.bytes_sent; r_null = stats.null_count; r_batch = stats.batches_sent; r_cols = icolinfo[0].colcount;
    ...
    short sl = strlen(r_stat); memcpy(r_stat + 258, &sl, 2); /* Store len at end for safety? No. */
    out->row->columnptr[0] = &r_amp; out->row->columnptr[1] = &r_rows; out->row->columnptr[2] = &r_bytes;
    out->row->columnptr[3] = &r_null; out->row->columnptr[4] = &r_batch; out->row->columnptr[5] = &r_cols;
    out->row->columnptr[6] = r_stat; out->row->lengths[0] = 4; out->row->lengths[1] = 8; out->row->lengths[2] = 8;
    ...
    for (i = 0; i < incount; i++) { FNC_free(icolinfo[i].iCols); FNC_TblOpClose(icolinfo[i].Handle); }
