...
void ExportToTrino(void) {
    ...
    /* stats.amp_id = FNC_GetAMPId(); // Use default 0 or find correct function */
    parse_params(&params);
    FNC_TblOpGetStreamCount(&incount, &outcount);
...
send_status:
    ...
    r_amp = stats.amp_id; r_rows = stats.rows_processed; r_bytes = stats.bytes_sent; r_null = stats.null_count; r_batch = stats.batches_sent; r_cols = total_input_cols;
    ...
    out->row->columnptr[0] = (void *)&r_amp; out->row->columnptr[1] = (void *)&r_rows; out->row->columnptr[2] = (void *)&r_bytes;
    out->row->columnptr[3] = (void *)&r_null; out->row->columnptr[4] = (void *)&r_batch; out->row->columnptr[5] = (void *)&r_cols;
    out->row->columnptr[6] = (void *)r_stat; out->row->lengths[0] = 4; out->row->lengths[1] = 8; out->row->lengths[2] = 8;
