...
public class TrinoExportMetadata implements io.trino.spi.connector.ConnectorMetadata {
    ...
    public Optional<io.trino.spi.connector.AggregationApplicationResult<io.trino.spi.connector.ConnectorTableHandle>> applyAggregation(
        ...
        // IMPORTANT: The order in projections must match the order in the SELECT list generated by the SplitSource.
        // First add grouping columns to projections, assignments, and mapping.
        for (io.trino.spi.connector.ColumnHandle groupCol : groupingColumns) {
            TrinoExportColumnHandle col = (TrinoExportColumnHandle) groupCol;
            projections.add(new io.trino.spi.expression.Variable(col.getName(), col.getType()));
            resultAssignments.add(new io.trino.spi.connector.Assignment(col.getName(), groupCol, col.getType()));
